...
lines(world) 
image.plot(lon, lat, meanqqB, zlim = zlim, col = paltas, main = "mean qqB")
lines(world) 
...
zlim  <- range(c(meanA, meanqqB, meanfakeB) - c(meanB)) %>% abs %>% max %>% "*"(., c(-1, 1))
par(mfrow = c(2, 2))
par(mar=c(5, 5, 5, 8))
image.plot(lon, lat, meanA - meanB, zlim = zlim, col = paldiff, main = "mean A-B")
lines(world) 
image.plot(lon, lat, meanqqB - meanB , zlim = zlim, col = paldiff, main = "mean qqB-B")
lines(world) 
...
sdqqB  <- t(apply(qqsetB, 2:3, sd))
sdfakeB  <- t(apply(fakesetB, 2:3, sd))
sdB  <- t(apply(datasetB, 2:3, sd))
zlim  <- range(sdA, sdqqB, sdfakeB, sdA)
...
lines(world) 
image.plot(lon, lat, sdqqB, zlim = zlim, col = paltas, main = "sd qqB")
lines(world) 
...
zlim  <- range(c(sdA, sdqqB, sdfakeB) - c(sdB)) %>% abs %>% max %>% "*"(., c(-1, 1))
...
lines(world) 
image.plot(lon, lat, sdqqB - sdB , zlim = zlim, col = paldiff, main = "sd qqB - sd B")
lines(world) 
...
ylim  <- range(spatial_rmse_qqrcpB, spatial_rmse_fakercpB) 
...
ylim  <- range(spatial_corr_qqrcpB, spatial_corr_fakercpB) 
...
zlim  <- range(temporal_corr_qqrcpB, temporal_corr_fakercpB) 
...
# }
# 
# zlim  <- range(c(qqrcpB, fakercpB) - c(rcpsetB)) %>% abs %>% max %>% "*"(., c(-1, 1))
